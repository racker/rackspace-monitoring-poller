...
func TestConnectionStream_WaitCh(t *testing.T) {
	type fields struct {
		ctx       context.Context
		stopCh    chan struct{}
		config    *config.Config
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := cs.WaitCh(); !reflect.DeepEqual(got, tt.expected) {
				t.Errorf("ConnectionStream.WaitCh() = %v, expected %v", got, tt.expected)
			}
		})
	}
}

func TestConnectionStream_connectBySrv(t *testing.T) {
	type fields struct {
		ctx       context.Context
		stopCh    chan struct{}
		config    *config.Config
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cs.connectBySrv(tt.args.qry)
		})
	}
}

func TestConnectionStream_connectByHost(t *testing.T) {
	type fields struct {
		ctx       context.Context
		stopCh    chan struct{}
		config    *config.Config
